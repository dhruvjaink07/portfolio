...
import { initRive } from './rive-manager.js';

// ===================================
// SHARED ASSETS & CONSTANTS
// ===================================
const SOCIAL_ICONS = {
    twitter: `<canvas id="rive-social-twitter" width="24" height="24" style="width: 24px; height: 24px; pointer-events: none;"></canvas>`,
    linkedin: `<canvas id="rive-social-linkedin" width="24" height="24" style="width: 24px; height: 24px; pointer-events: none;"></canvas>`,
    github: `<canvas id="rive-social-github" width="24" height="24" style="width: 24px; height: 24px; pointer-events: none;"></canvas>`,
    instagram: `<canvas id="rive-social-instagram" width="24" height="24" style="width: 24px; height: 24px; pointer-events: none;"></canvas>`
};
    ...
    if (experienceList) {
        experienceList.innerHTML = workExperience.map(exp => `
            <div class="experience-item">
                ...
                <div class="experience-description" style="margin-top: 12px; line-height: 1.6; opacity: 0.9;">
                    ${exp.description.includes('•') ? 
                        `<ul style="padding-left: 20px; margin: 0; display: flex; flex-direction: column; gap: 8px; list-style-type: disc;">
                            ${exp.description.split('\n').map(line => 
                                line.trim().startsWith('•') ? `<li>${line.replace('•', '').trim()}</li>` : `<li style="list-style: none; margin-left: -20px;">${line}</li>`
                            ).join('')}
                        </ul>` : 
            ...
            <div class="experience-item">
                ...
                <div class="experience-description" style="margin-top: 12px; line-height: 1.6; opacity: 0.9;">
                    ${edu.description.includes('•') ? 
                        ...
                        `<p style="margin: 0; white-space: pre-line;">${edu.description}</p>`
                    }
                </div>
            </div>
        `).join('');
...
// ===================================
window.viewProjectsBySkill = function viewProjectsBySkill(skillName) {
